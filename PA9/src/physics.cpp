...
void Physics::Update(unsigned int dt, int keyboardButton)
{
  ...
  cylinder->Update(dt, keyboardButton);
  cube->Update(dt, keyboardButton);
  btTransform newTrans;
  if(keyboardButton == SDLK_UP)
  {
    cube->rigidBody->getMotionState()->getWorldTransform(newTrans);
    newTrans.getOrigin() += (btVector3(0, 0, .5f));
    cube->rigidBody->getMotionState()->setWorldTransform(newTrans);
  }
  if(keyboardButton == SDLK_DOWN)
  {
    cube->rigidBody->getMotionState()->getWorldTransform(newTrans);
    newTrans.getOrigin() += (btVector3(0, 0, -.5f));
    cube->rigidBody->getMotionState()->setWorldTransform(newTrans);
  }
  if(keyboardButton == SDLK_LEFT)
  {
    cube->rigidBody->getMotionState()->getWorldTransform(newTrans);
    newTrans.getOrigin() += (btVector3(.5f, 0, 0));
    cube->rigidBody->getMotionState()->setWorldTransform(newTrans);
  }
  if(keyboardButton == SDLK_RIGHT)
  {
    cube->rigidBody->getMotionState()->getWorldTransform(newTrans);
    newTrans.getOrigin() += (btVector3(-.5f, 0, 0));
    cube->rigidBody->getMotionState()->setWorldTransform(newTrans);
  }
  if(keyboardButton == SDLK_RETURN) //up
  {
    cube->rigidBody->getMotionState()->getWorldTransform(newTrans);
    newTrans.getOrigin() += (btVector3(0, .5f, 0));
    cube->rigidBody->getMotionState()->setWorldTransform(newTrans);
  }
  if(keyboardButton == SDLK_RCTRL) //down
  {
    cube->rigidBody->getMotionState()->getWorldTransform(newTrans);
    newTrans.getOrigin() += (btVector3(0, -0.5f, 0));
    cube->rigidBody->getMotionState()->setWorldTransform(newTrans);
  }
